...
\movie[width=3cm,height=2cm,poster]{}{Infinitesimal simulation.avi}
...
\begin{center}
\begin{tikzpicture}
\draw plot[smooth] coordinates{(0,2)(1,2.5)(2,2)(1.75,1.1)(1,1)(2,2)(3,2.75)(5,3)};
\draw[->](0,0)node[left]{O} -- (1,2.5)node[left]{$\vec{x}(t_0)$};
\draw[->](0,0) -- (4,2.9) node[below]{$\vec{x}(t_0 + \Delta t)$};
\draw[->](1,2.5) -- (4,2.9)node[above]{$\Delta x$};
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}
\draw[->] (0,5,0) node[left]{O} -- (0,5,-1)node[left]{$\vec{e_2}$};
\draw[->] (0,5,0) -- (1,5,0) node[right]{$\vec{e_1}$};
\draw[->] (0,5,0) -- (0,4,0) node[left]{$\vec{e_3}$};
\begin{axis} [
view={5}{50},
axis lines=none,
ymin=-5,
...
xmin=-2,
xmax=2]
\addplot3 [domain=.75*pi:8.25*pi, samples = 100, samples y=0] ({sin(deg(-x))}, {cos(deg(-x))}, {x});
\end{axis}
\end{tikzpicture}
\end{center}
\begin{multicols}{2}
